...
def run_lrgs(x,y,err_x,err_y,xycov=None,nmc=500,dirichlet=True):
    '''Runs the lrgs regression algorithm written in R by interfacing through rpy2.
       For our purposes, inputs should be in scaled (log) form. (For the moment,
       only works for on-diagonal elements of the covariance matrix.) nmc is the
    ...
    NULL = robjects.rinterface.NULL # Not needed at the moment
    ...
    # Run MCMC
    posterior = rlrgs.Gibbs_regression(rx, ry, M, nmc, dirichlet=d, trace='bsg',mention_every=50)

    # Extract relevant data from posterior
    B = np.array(posterior[0]) # Parameter chain
    S = np.array(posterior[1])[0][0] # Scatter chain (only intrinsic scatter for the moment!)
    G = np.array(posterior[2]) # Gaussian mixture model chain
...
def run_linmix(x,y,err_x,err_y,Nmin=5000,Nmax=10000,vb=False):
...
def check_convergence(intercept,slope,sigma):
    '''
    FIX: In future, should implement a function that checks the convergence of the MCMC using
    autocorrelation, etc. and display plots/statistics.
    '''
    pass
